...
if (tilemap == -1) {
    ...
    if (tilemap == -1) {
        ...
        if (instance_exists(obj_enemy_spawner) && obj_enemy_spawner.tilemap != -1) {
            ...
            show_debug_message("Enemy1 got tilemap from spawner: " + string(tilemap));
        } else {
...
show_debug_message("Enemy1 ID:" + string(id) + " | Distance to player: " + string(_dist_to_player) + 
...
// Debug state and position
show_debug_message("Enemy1 state: " + string(state) + " | Position: " + string(x) + "," + string(y));

// Determine state based on player distance
if (_dist_to_player <= detection_radius) {
    state = "chase";
    show_debug_message("Enemy1 ID:" + string(id) + " entering CHASE state");
} else {
